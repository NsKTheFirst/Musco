const userModel = function userModel(connection) {
  ...
  const table = "users";
    ...
    const create = function createUser(clbk, payload) {
      const sql = `INSERT INTO ${table} (nom, prenom, pseudo, mdp, mail, avatar, soundcloud, youtube, facebook) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      const { nom, prenom, pseudo, avatar, mail, mdp, soundcloud, youtube, facebook } = payload;
      const query = connection.query(
        sql,
        [nom, prenom, pseudo, mdp, mail, avatar, soundcloud, youtube, facebook],
        (err, res) => {
    ...
    const get = function getFullUser(clbk, id_user) {
      ...
      if (id_user && !isNaN(id_user)) {
        sql = "SELECT *, m.date AS 'date', m.sujet, m.message, m.id_emetteur, a.annonce, a.date as 'a_date', a.localisation, ufu.id_user_followed FROM users AS u LEFT JOIN messages AS m ON u.id_user = m.id_receveur LEFT JOIN annonces AS a ON u.id_user = a.id_annonce_owner LEFT JOIN user_follows_user AS ufu ON u.id_user = ufu.id_user_followed WHERE u.id_user = ?";
      } else {
    ...
    const getByMail = function getUserByMail(clbk, mail) {
      const sql = `SELECT *, m.date AS 'date', m.sujet, m.message, m.id_emetteur, a.annonce, a.date as 'a_date', a.localisation, ufu.id_user_followed FROM ${table} AS u LEFT JOIN messages AS m ON u.id_user = m.id_receveur LEFT JOIN annonces AS a ON u.id_user = a.id_annonce_owner LEFT JOIN user_follows_user AS ufu ON u.id_user = ufu.id_user_followed WHERE u.mail = ?`;
